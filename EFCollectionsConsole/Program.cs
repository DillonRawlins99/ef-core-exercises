...
namespace EFCollectionsConsole
{
    class Program
    {
        ...
        static bool UpdateData(TrainingContext db) 
        {
            var toUpdate = db.Courses.AsQueryable().Where(e => e.Title == "Introduction to EF Core").Single();
            toUpdate.Title = "Introduction to EF Core V2";
            toUpdate.Author = new Teacher { Name = "Dillon Rawlins" };
            db.SaveChanges();
            return true;
        }
